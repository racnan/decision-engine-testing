...
def validate_config(config):
    ...
    assert (
        "num_transactions" in config["simulation"]
    ), "Config missing 'num_transactions'."
    assert isinstance(
        config["simulation"]["num_transactions"], int
    ), "'num_transactions' must be an integer."

    assert "schema" in config, "Config missing 'schema' section."
    assert (
        "transaction_fields" in config["schema"]
    ), "Schema missing 'transaction_fields'."
    assert isinstance(
        config["schema"]["transaction_fields"], list
    ), "'transaction_fields' must be a list."
    ...
    for event in config.get("event_schedule", []):
        ...
        if isinstance(transaction_range, list) and len(transaction_range) == 2:
            ...
            if start <= entry_number <= end:
                ...
                structured_pmt = None
                if payment_method_type_data and isinstance(
                    payment_method_type_data, dict
                ):
                    structured_pmt = {
                ...
                if amount_data and isinstance(amount_data, dict):
                    amount = {"gt": amount_data.get("gt"), "lt": amount_data.get("lt")}
...
def get_current_processor_state(config, entry_number, processor_name):
    ...
    processor_defaults = next(
        (p["defaults"] for p in config["processors"] if p["name"] == processor_name),
        None,
    )
...
def generate_random_choice(params):
    ...
    try:
        ...
        if len(primary_weights) != len(primary_values):
            raise ValueError(
                "Length of 'primary_weights' must match length of 'primary_values'."
            )
        ...
        if "combinations" in params:
            ...
            if chosen_pool == "primary":
                ...
                if len(combination_weights) != len(combinations):
                    raise ValueError(
                        "Length of 'combination_weights' must match 'combinations'."
                    )
        ...
        raise KeyError(
            f"A required parameter (e.g., 'primary_values' or a weight) is missing: {e}"
        )
        ...
        for field_config in fields:
            ...
            # If it's not a conditional generator, it's a standard data field.
                ...
                if generator_type == "random_number":
                    ...
                    raise ValueError(
                        f"Unknown generator type '{generator_type}' for field '{field_name}'."
                    )
...
def discover_existing_scenes():
    ...
    for item in os.listdir(project_root):
        if os.path.isdir(os.path.join(project_root, item)) and item.startswith(
            "scene-"
        ):
            try:
...
def generate_transactions_for_scene(scene_number):
    ...
    try:
        with open(output_path, "w", newline="") as f:
            ...
            print(
                f"[SCENE {scene_number}] Generating {num_transactions} transactions..."
            )

            for i in range(1, num_transactions + 1):
                if i % 1000 == 0:
                    print(
                        f"[SCENE {scene_number}] Progress: {i}/{num_transactions} transactions"
                    )
                ...
                for processor in config["processors"]:
                    ...
                    for network in state.get("supported_networks", []):
                        ...
                        for event in active_events:
                            ...
                            if check_transaction_against_event(
                                row_data, event, processor_name, network
                            ):
        ...
        print(
            f"[SCENE {scene_number}] ✓ Successfully generated transactions: {output_path}"
        )
        return True
...
def main():
    ...
    if args.scenes:
        ...
        if missing_scenes:
            ...
            print(
                "Please ensure schema.yaml exists in each scene folder before generating transactions."
            )
            sys.exit(1)
        ...
        if successful_scenes:
            print(
                f"✓ Successfully generated transactions for: {', '.join(f'scene-{num}' for num in successful_scenes)}"
            )

        if failed_scenes:
            print(
                f"✗ Failed to generate transactions for: {', '.join(f'scene-{num}' for num in failed_scenes)}"
            )
            sys.exit(1)
        ...
        try:
            with open(output_path, "w", newline="") as f:
                ...
                writer.writeheader()
                print(
                    f"INFO: Successfully created CSV file and wrote headers to: {output_path}"
                )

                num_transactions = config["simulation"]["num_transactions"]
                print(
                    f"INFO: Starting generation of {num_transactions} transactions..."
                )
