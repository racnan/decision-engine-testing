...
def analyze_results(input_file=None, output_dir=None, return_metrics=False):
    """
    ...
    Args:
        input_file: Path to the CSV results file
        output_dir: Directory to output any files to
        return_metrics: If True, return a dict of metrics instead of None

    Returns:
        Dict of metrics if return_metrics=True, otherwise None
    """
    ...
    if not os.path.exists(results_path):
        ...
        print(
            "INFO: Please run the run_simulations.py script first to generate results."
        )
        return
    ...
    try:
        with open(results_path, "r") as f:
            ...
            for row in reader:
                ...
                is_optimal = row.get("chosen_processor") == row.get(
                    "best_possible_processor"
                ) and row.get("chosen_network") == row.get("best_possible_network")
    ...
    print(
        f"    # The total number of transactions that were processed from the input file."
    )
    ...
    print(
        f"  - Failed Transactions:          {total_transactions - successful_transactions}"
    )
    ...
    if not processor_stats:
        ...
        sorted_processors = sorted(
            processor_stats.items(), key=lambda item: item[1]["total"], reverse=True
        )
        total_choices = sum(p["total"] for p in processor_stats.values())

        for processor, stats in sorted_processors:
            percentage = (
                (stats["total"] / total_choices) * 100 if total_choices > 0 else 0
            )
            ...
            if stats["networks"]:
                ...
                sorted_networks = sorted(
                    stats["networks"].items(),
                    key=lambda item: item[1]["total"],
                    reverse=True,
                )
                for network, net_stats in sorted_networks:
                    net_percentage = (
                        (net_stats["total"] / stats["total"]) * 100
                        if stats["total"] > 0
                        else 0
                    )
                    print(
                        f"      - {network:<10}: {net_stats['total']} times ({net_percentage:.2f}%)"
                    )
                    print(
                        f"        - Success: {net_stats['success']}, Fail: {net_stats['fail']}"
                    )

    # --- Network Choice Distribution ---
    print("\nNetwork Choice Distribution:")
    print(
        "    # Breaks down how many times each network was chosen and its performance."
    )
    if not network_stats:
        print("  - No networks were chosen during the simulation.")
    else:
        sorted_networks = sorted(
            network_stats.items(), key=lambda item: item[1]["total"], reverse=True
        )
        total_network_choices = sum(n["total"] for n in network_stats.values())

        for network, stats in sorted_networks:
            percentage = (
                (stats["total"] / total_network_choices) * 100
                if total_network_choices > 0
                else 0
            )
            print(
                f"    # The number and percentage of times this network was selected."
            )
    ...
    print(
        "    # Evaluates the performance of the routing logic against a perfect outcome."
    )
    ...
    print(
        f"    - Best Possible Savings:            ${best_possible_savings_success:,.2f}"
    )
    ...
    print(
        f"      # This is the total savings that could have been realized if these transactions had succeeded via their optimal route."
    )
    ...
    print("\n  --- Overall Summary ---")
    print(
        f"    - Total Potential Savings Missed (for Successful and Failed transactions):     ${total_missed_savings:,.2f}"
    )
    ...
    if return_metrics:
        return {
            ...
            "achieved_savings": achieved_savings_success,
            "best_possible_savings": best_possible_savings_success,
...
def create_csv_performance_report(input_file=None, output_dir=None):
    ...
    if output_dir:
        ...
        reports_folder = os.path.join(
            project_root, "scene-1", "detailed_performance_analysis"
        )

    if not os.path.exists(results_path):
        print(
            f"ERROR: Cannot create CSV reports - results file not found at {results_path}"
        )
        return
    ...
    overall_data = [
        ...
        [
            ...
            optimal_choices,
            f"{(optimal_choices/total_transactions)*100:.2f}%",
            "100.00%",
        ],
    ]
    ...
    if processor_stats:
        ...
        try:
            processor_csv_path = os.path.join(
                reports_folder, "processor_performance.csv"
            )
    ...
    for processor, stats in sorted(
        ...
        if stats["networks"]:
            ...
            for network, net_stats in sorted_networks:
                ...
                success_rate = (
                    (net_stats["success"] / net_stats["total"]) * 100
    ...
    if len(breakdown_data) > 1:  # More than just headers
        try:
            breakdown_csv_path = os.path.join(
                reports_folder, "processor_network_breakdown.csv"
            )
...
def create_detailed_performance_report(input_file=None, output_dir=None):
    ...
    if output_dir:
        ...
        report_path = os.path.join(
            project_root, "scene-1", "detailed_performance_report.txt"
        )

    if not os.path.exists(results_path):
        print(
            f"ERROR: Cannot create detailed report - results file not found at {results_path}"
        )
        return
    ...
    report_content.append(
        generate_ascii_table(overall_headers, overall_rows, "OVERALL PERFORMANCE")
    )
    ...
    if processor_stats:
        ...
        report_content.append(
            generate_ascii_table(
                processor_headers, processor_rows, "PROCESSOR PERFORMANCE"
            )
        )
    ...
    if network_stats:
        ...
        report_content.append(
            generate_ascii_table(network_headers, network_rows, "NETWORK PERFORMANCE")
        )
    ...
    if breakdown_rows:
        report_content.append(
            generate_ascii_table(
                breakdown_headers, breakdown_rows, "PROCESSOR-NETWORK BREAKDOWN"
            )
        )
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Generate results analysis and reports"
    )
    parser.add_argument("--input-file", type=str, help="Path to input results CSV file")
