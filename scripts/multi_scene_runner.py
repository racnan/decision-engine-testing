...
def setup_redis_connection():
    ...
    try:
        ...
        print(
            f"✗ FATAL: Cannot connect to Redis at {REDIS_CONFIG['host']}:{REDIS_CONFIG['port']}"
        )
        ...
        if "-" in part and len(part.split("-")) == 2:
            ...
            try:
                ...
                for i in range(start_num, end_num + 1):
                    ...
                    if (
                        scene_name in available_scenes
                        and scene_name not in selected_scenes
                    ):
...
def generate_run_reports(scene_folder, run_dir):
    ...
    try:
        ...
        cmd = [
            ...
            script_path,
            "--input-file",
            os.path.join(run_dir, "output_results.csv"),
            "--output-dir",
            run_dir,
        ]
    ...
    for run_num in range(1, num_runs + 1):
        ...
        if os.path.exists(results_file):
            ...
            scene_results["run_details"].append(
                {"run_number": run_num, "status": "success", "stats": run_stats}
            )

            # Aggregate stats
            agg = scene_results["aggregate_stats"]
            agg["total_transactions"] += run_stats.get("total_transactions", 0)
            agg["successful_transactions"] += run_stats.get(
                "successful_transactions", 0
            )
            ...
            scene_results["run_details"].append(
                {"run_number": run_num, "status": "failed", "stats": {}}
            )
    ...
    try:
        with open(results_file, "r") as f:
            ...
            for row in reader:
                ...
                try:
                    stats["best_possible_savings"] += float(
                        row.get("best_possible_savings", 0.0)
                    )
                except (ValueError, TypeError):
                    pass
...
def print_cross_scene_summary(all_scene_results, algorithm):
    ...
    total_transactions = sum(
        scene["aggregate_stats"]["total_transactions"] for scene in all_scene_results
    )
    total_successful_transactions = sum(
        scene["aggregate_stats"]["successful_transactions"]
        for scene in all_scene_results
    )
    ...
    if total_transactions > 0:
        overall_success_rate = (
            total_successful_transactions / total_transactions
        ) * 100
    ...
    for scene_result in all_scene_results:
        ...
        if stats["total_transactions"] > 0:
            scene_success_rate = (
                stats["successful_transactions"] / stats["total_transactions"]
            ) * 100
            print(
                f"  Transactions: {stats['successful_transactions']:,}/{stats['total_transactions']:,} ({scene_success_rate:.2f}%)"
            )
            print(
                f"  Savings: ${stats['total_savings']:.2f} / ${stats['best_possible_savings']:.2f}"
            )

            if stats["best_possible_savings"] > 0:
                efficiency = (
                    stats["total_savings"] / stats["best_possible_savings"]
                ) * 100
    ...
    if all_scene_results:
        best_scene = max(
            all_scene_results,
            key=lambda x: (
                x["aggregate_stats"]["successful_transactions"]
        ...
        if best_scene_stats["total_transactions"] > 0:
            best_success_rate = (
                best_scene_stats["successful_transactions"]
                / best_scene_stats["total_transactions"]
            ) * 100
            print(
...
def generate_consolidated_summary(scenes, algorithm, num_runs):
    ...
    try:
        ...
        script_path = os.path.join(
            project_root, "scripts", "generate_summary_report.py"
        )
